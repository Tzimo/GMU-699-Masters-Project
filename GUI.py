...
packages = ['nltk', 'keras', 'tensorflow', 'tensorflow_hub', 'sentencepiece', 'docx2pdf', 'PyPDF2', 'xlsxwriter', 'PyQt5']
...
import os, csv, tokenization, json, datetime, docx, logging, ssl, string, re
...
class App(QWidget):
    ...
    def open_report(self):
        if os.path.isfile(self.output_file):
            cmd = 'start excel ' + '\"' + self.output_file + '\"'
            os.system(cmd)
        else:
